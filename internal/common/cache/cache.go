...
func NewIMCache(conf *config.Config) *IMCache {
	loader := ttlcache.LoaderFunc[string, string](
		func(c *ttlcache.Cache[string, string], key string) *ttlcache.Item[string, string] {
			...
			_token_path := idp_config.Url + "/realms/" + idp_config.RealmName + "/protocol/openid-connect/token"
			...
			response, err := restyClient.R().SetFormData(data).
				...
				Post(_token_path)
			if err != nil {
				log.Error().Err(err).Caller().Msg("Error while getting token")
				return nil
			}
			if response.IsError() {
				log.Error().Str("response", string(response.Bytes())).Caller().Msg("Error while getting token")
				return nil
			}
